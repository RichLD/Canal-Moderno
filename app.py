...
def cargar_datos(uploaded_bytes: bytes) -> dict:
    import io
    df_raw = pd.read_csv(io.BytesIO(uploaded_bytes), low_memory=False)
...
with st.sidebar:
    ...
    uploaded_file = st.file_uploader(
        ...
        type=['csv'],
        help='Sube el archivo SUPERMERCADOS en formato CSV.',
    )
    ...
    # Cargar y procesar (cacheado por hash del archivo)
    datos = cargar_datos(uploaded_file.read())
