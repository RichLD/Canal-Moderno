...
@st.cache_data(show_spinner='Cargando datos 2026...')
def cargar_base() -> pd.DataFrame:
    df = pd.read_csv('BASE/base_2026.csv.gz', compression='gzip', low_memory=False)
    df.columns = df.columns.str.strip()
    cols_str = ['Cadena', 'Descripcion', 'MARCA', 'CATEGORIA', 'Formato',
                'NomTienda', 'Ciudad', 'Estado', 'Vendedores']
    for col in cols_str:
...
@st.cache_data(show_spinner='Cargando resumen 2025...')
def cargar_resumen_2025() -> pd.DataFrame:
    df = pd.read_csv('BASE/resumen_2025.csv.gz', compression='gzip')
    df.columns = df.columns.str.strip()
    return df
...
with st.sidebar:
    ...
    df_raw = cargar_base()
    df_2025 = cargar_resumen_2025()

    st.success(f'✅ {len(df_raw):,} registros 2026')
    st.markdown('---')
    st.markdown('### Filtros')
    ...
    if 'Vendedores' in df_raw.columns:
        vend_disp = sorted(df_raw['Vendedores'].unique().tolist())
...
def filtrar(cadenas, marcas, estados, vendedores):
    mask = (
        df_raw['Cadena'].isin(cadenas) &
        df_raw['MARCA'].isin(marcas) &
        df_raw['Estado'].isin(estados)
    )
    if vendedores and 'Vendedores' in df_raw.columns:
        mask = mask & df_raw['Vendedores'].isin(vendedores)
    return df_raw[mask].copy()


base = filtrar(tuple(cadenas_sel), tuple(marcas_sel), tuple(estados_sel), tuple(vend_sel))

if base.empty:
    st.warning('⚠️ No hay datos con los filtros seleccionados.')
    st.stop()

kpis    = calcular_kpis(base, df_2025, cadenas_sel, vend_sel)
sem_max = int(base['SEMANA'].max()) if 'SEMANA' in base.columns else 0

c1, c2 = st.columns([3, 1])
with c1:
    st.markdown('# 📊 Dashboard Sell Out — Arcor')
    st.caption(f'Semana: **2026 — S{sem_max:02d}** &nbsp;|&nbsp; '
               f'{base["NomTienda"].nunique():,} tiendas &nbsp;|&nbsp; {base["Cadena"].nunique()} cadenas')
with c2:
...
if 'Es_Agotado' in base.columns:
    ...
    exceso_cnt     = int(base['Exceso_Stock'].sum()) if 'Exceso_Stock' in base.columns else 0
    if agotados_cnt > 0:
        st.markdown(f"""
        <div class="alert-banner">
            🔴 <b>{agotados_cnt:,} tiendas con agotados</b> que generaron
            <b>${agotados_venta/1e6:.1f}M</b> &nbsp;|&nbsp;
            ⚠️ <b>{exceso_cnt:,} registros con exceso de stock</b>
        </div>""", unsafe_allow_html=True)
...
if kpis['var_yoy'] is not None:
    ...
    signo = '+' if kpis['var_yoy'] >= 0 else ''
    delta_html = f'<span class="{cls}">{signo}{kpis["var_yoy"]:.1f}% vs 2025</span>'

kpi_card(k1, 'Venta Total 2026', f'${kpis["venta_actual"]/1e6:.1f}M', delta_html)
...
kpi_card(k5, 'Tiendas', f'{base["NomTienda"].nunique():,}', '')
kpi_card(k6, 'SKUs', f'{base["UPC"].nunique():,}', '')
...
with tab2:
    st.plotly_chart(grafica_yoy(base, df_2025, cadenas_sel, vend_sel), use_container_width=True)
...
with tab3:
    abc_fig, abc_tabla = grafica_abc(base)
    c1, c2 = st.columns([3, 2])
    with c1:
    ...
    with st.expander('📋 Tabla completa ABC'):
        st.dataframe(abc_tabla, use_container_width=True, height=350)
...
st.download_button('⬇️ Base filtrada (.csv)',
